...
function getZonesService(): ZonesService {
  try {
    ...
    throw new Error(
      `Failed to initialize ZonesService: ${
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        error:
          "All fields are required: name, zoneType, boundaries, maxAltitude, minAltitude, description",
      });
    }
...
router.put("/update/:id", async (req, res) => {
  try {
    ...
    const { name, boundaries, maxAltitude, minAltitude, description } =
      req.body;
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        error:
          "All fields are required: name, boundaries, maxAltitude, minAltitude, description",
      });
    }
  ...
  try {
    ...
    if (!(zoneType in ZoneType)) {
      return res.status(400).json({
        success: false,
        error:
          "Invalid zone type. Valid types: RURAL, URBAN, HOSPITALS, MILITARY, RESTRICTED",
      });
    }
    ...
    res.json({
      ...
      data: { zoneIds },
      message: `${
        activeOnly ? "Active zones" : "Zones"
      } by type retrieved successfully`,
    });
  } catch (error) {
