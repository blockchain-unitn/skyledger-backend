...
export class DronesService {
  ...
  private formatDroneForResponse(
    tokenId: number,
    drone: Drone,
    owner: string
  ): DroneResponse {
  ...
  async mintDrone(
    droneData: MintDroneRequest
  ): Promise<{ tokenId: number; txHash: string }> {
    const recipientAddress = this.validateAndFormatAddress(
      droneData.ownerHistory[0]
    );
  ...
  async updateCertHashes(updateData: UpdateCertHashesRequest): Promise<string> {
    const tx = await this.contract.updateCertHashes(
      updateData.tokenId,
      updateData.certHashes
    );
    await tx.wait();
    return tx.hash;
  }

  async updatePermittedZones(
    updateData: UpdatePermittedZonesRequest
  ): Promise<string> {
    const tx = await this.contract.updatePermittedZones(
      updateData.tokenId,
      updateData.permittedZones
    );
    await tx.wait();
    return tx.hash;
  }

  async updateOwnerHistory(
    updateData: UpdateOwnerHistoryRequest
  ): Promise<string> {
    const tx = await this.contract.updateOwnerHistory(
      updateData.tokenId,
      updateData.ownerHistory
    );
    await tx.wait();
    return tx.hash;
  }

  async updateMaintenanceHash(
    updateData: UpdateMaintenanceHashRequest
  ): Promise<string> {
    const tx = await this.contract.updateMaintenanceHash(
      updateData.tokenId,
      updateData.maintenanceHash
    );
    await tx.wait();
    return tx.hash;
  }
