...
export class ViolationsAlertingService {
  ...
  constructor() {
    ...
    if (!contractAddress) {
      throw new Error(
        "VIOLATIONS_ALERTING_ADDRESS environment variable is required"
      );
    }
    ...
    const abi = [
      "function reportViolation(uint256  droneID, string memory position) public",
  ...
  async reportViolation(
    ...
    try {
      ...
      const tx = await this.contract.reportViolation(
        request.droneID,
        request.position
      );
  ...
  async getViolationsCount(): Promise<number> {
    try {
      ...
      throw new Error(
        `Failed to get violations count: ${
          error instanceof Error ? error.message : "Unknown error"
        }`
      );
    }
  ...
  async getViolationsByDrone(
    droneID: string
  ): Promise<ViolationsByDroneResponse> {
    try {
      ...
      throw new Error(
        `Failed to get violations by drone: ${
          error instanceof Error ? error.message : "Unknown error"
        }`
      );
    }
